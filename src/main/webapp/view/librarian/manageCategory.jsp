...
<%
    ...
    String activeTab = (request.getParameter("query") != null || "category_updated".equals(msg) || "category_deleted".equals(msg)) ? "list" : "register";
%>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
<body>
  ...
  <main class="container-fluid categories-container">
    ...
    <div class="toast-msg toast-success <%= "category_added".equals(msg) ? "show" : "" %>">
      <i class="bi bi-check-circle-fill"></i> Categoria cadastrada com sucesso!
    </div>
    <div class="toast-msg toast-success <%= "category_updated".equals(msg) ? "show" : "" %>">
      <i class="bi bi-check-circle-fill"></i> Alterações salvas com sucesso!
    </div>
    <div class="toast-msg toast-success <%= "category_deleted".equals(msg) ? "show" : "" %>">
      <i class="bi bi-check-circle-fill"></i> Categoria removida com sucesso!
    </div>
    <div class="toast-msg toast-error <%= "error".equals(msg) ? "show" : "" %>">
      <i class="bi bi-exclamation-circle-fill"></i> Erro ao processar a requisição.
    </div>

    <%-- Abas: Atributos onclick removidos para evitar erros de validação no editor --%>
    <div class="tabs">
      <button type="button" class="tab-btn <%= "register".equals(activeTab) ? "active" : "" %>" data-target="register">Cadastrar categoria</button>
      <button type="button" class="tab-btn <%= "list".equals(activeTab) ? "active" : "" %>" data-target="list">Categorias cadastradas</button>
    </div>
          ...
          <% for (Category cat : categories) { %>
            <div class="category-card" id="card-<%= cat.getCategoryId() %>">
              ...
              <div class="category-info">
                ...
                <%-- Botão Editar modificado: removido onclick inline e inserido data-id --%>
                ...
                <form action="${pageContext.request.contextPath}/manageCategory" method="post" class="edit-form" id="form-edit-<%= cat.getCategoryId() %>">
                  ...
                  <%-- Botão Cancelar modificado: removido onclick inline e inserido data-id --%>
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function() {
      ...
      // Gerenciamento de Abas limpo (Sem JS misturado com HTML)
      ...
      // Ativação do formulário inline de edição
      ...
      // Cancelamento do formulário inline de edição
      ...
      // Controle de visibilidade das notificações Toast
