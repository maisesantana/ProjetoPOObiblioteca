    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("profile".equals(action)) {
            ...
            try {
                ...
                request.getRequestDispatcher("/view/attendant/clientProfile.jsp")
                       .forward(request, response);
            } catch (Exception e) {
                response.sendRedirect(request.getContextPath()
                    + "/clients?msg=not_found");
            }
            return;
        }
        ...
        if ("register".equals(action)) {
            request.getRequestDispatcher("/view/attendant/registerClient.jsp")
                   .forward(request, response);
            return;
        }
        ...
        if ("edit".equals(action)) {
            ...
            try {
                Client client = clientService.findByCpf(cpf)
                    .orElseThrow(() -> new IllegalArgumentException("Cliente não encontrado."));
                request.setAttribute("client", client);
                request.getRequestDispatcher("/view/attendant/editClient.jsp")
                       .forward(request, response);
            } catch (Exception e) {
                response.sendRedirect(request.getContextPath()
                    + "/clients?msg=not_found");
        ...
        // Lista todos ou busca por nome/CPF
        try {
            List<Client> clients;
            if (query != null && !query.trim().isEmpty()) {
                // tenta buscar por CPF primeiro, depois por nome
        ...
        request.getRequestDispatcher("/view/attendant/clients.jsp")
               .forward(request, response);
    }
